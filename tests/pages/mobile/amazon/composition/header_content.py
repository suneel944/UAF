...
class HeaderContent(BasePage):
    ...
    # Elements
    __BTN_HAMBURGER_MENU: tuple[str, str] = (
        AppiumBy.XPATH,
        ".//*[@id='nav-hamburger-menu']",
    )
    __ELE_FILTER_MENU: tuple[str, str] = (
        AppiumBy.XPATH,
        ".//*[@data-csa-c-content-id='s-all-filters']",
    )
    __LNK_SHOW_RESULTS: tuple[str, str] = (
        AppiumBy.XPATH,
        "//*[contains(text(),'result')]//parent::a",
    )

    def __tab_filter_panel_selection(self, tab_name: str) -> tuple[str, str]:
        return (
            AppiumBy.XPATH,
            f".//*[@id='{tab_name}']//*[@data-action='s-vtab-action']",
        )
    ...
    def filter_product_based_on_specifics(self, tab_name: str, filter_specific: str):
        ...
        self.wait.wait_for_element_visibility(
            self.__tab_filter_panel_selection(tab_name)
        )
    ...
    def select_left_menu_specifics(
        ...
        self.element.click_on_element_using_js(
            self.__lnk_left_panel_menu_selection(sub_section)
        )
        return self
